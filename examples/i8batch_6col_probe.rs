...
mod x86_probe {
    ...
    fn dot_i8(w: &[i8], x: &[i8]) -> i32 {
        ...
        unsafe {
            ...
            while i + 32 <= n {
                ...
                a0 = _mm256_add_epi32(
                    a0,
                    _mm256_madd_epi16(w0, _mm256_cvtepi8_epi16(_mm_loadu_si128(xp.add(i).cast()))),
                );
                a1 = _mm256_add_epi32(
                    a1,
                    _mm256_madd_epi16(
                        w1,
                        _mm256_cvtepi8_epi16(_mm_loadu_si128(xp.add(i + 16).cast())),
                    ),
                );
                i += 32;
            }
    ...
    fn dot_i8_4col(w: &[i8], xa: &[i8], xb: &[i8], xc: &[i8], xd: &[i8]) -> (i32, i32, i32, i32) {
        ...
        unsafe {
            ...
            let (mut sa, mut sb) = (
                hsum256(_mm256_add_epi32(aa0, aa1)),
                hsum256(_mm256_add_epi32(ab0, ab1)),
            );
            let (mut sc, mut sd) = (
                hsum256(_mm256_add_epi32(ac0, ac1)),
                hsum256(_mm256_add_epi32(ad0, ad1)),
            );
            while i < n {
                let wv = *w.get_unchecked(i) as i32;
                sa += wv * *xa.get_unchecked(i) as i32;
                sb += wv * *xb.get_unchecked(i) as i32;
                sc += wv * *xc.get_unchecked(i) as i32;
                sd += wv * *xd.get_unchecked(i) as i32;
                i += 1;
            }
            (sa, sb, sc, sd)
        }
    ...
    fn dot_i8_6col(
        ...
        unsafe {
            ...
            while i + 32 <= n {
                ...
                ae0 = _mm256_add_epi32(
                    ae0,
                    _mm256_madd_epi16(w0, _mm256_cvtepi8_epi16(_mm_loadu_si128(ep.add(i).cast()))),
                );
                ae1 = _mm256_add_epi32(
                    ae1,
                    _mm256_madd_epi16(
                        w1,
                        _mm256_cvtepi8_epi16(_mm_loadu_si128(ep.add(i + 16).cast())),
                    ),
                );
                af0 = _mm256_add_epi32(
                    af0,
                    _mm256_madd_epi16(w0, _mm256_cvtepi8_epi16(_mm_loadu_si128(fp.add(i).cast()))),
                );
                af1 = _mm256_add_epi32(
                    af1,
                    _mm256_madd_epi16(
                        w1,
                        _mm256_cvtepi8_epi16(_mm_loadu_si128(fp.add(i + 16).cast())),
                    ),
                );
                i += 32;
            }
            ...
            let (mut se, mut sf) = (
                hsum256(_mm256_add_epi32(ae0, ae1)),
                hsum256(_mm256_add_epi32(af0, af1)),
            );
            while i < n {
                let wv = *w.get_unchecked(i) as i32;
                sa += wv * *xa.get_unchecked(i) as i32;
                ...
                sc += wv * *xc.get_unchecked(i) as i32;
                sd += wv * *xd.get_unchecked(i) as i32;
                se += wv * *xe.get_unchecked(i) as i32;
                sf += wv * *xf.get_unchecked(i) as i32;
                i += 1;
            }
            (sa, sb, sc, sd, se, sf)
        }
            ...
            .map(|&(o0, o1)| {
                ...
                for o in o0..o1 {
                    ...
                    while t + 4 <= tq {
                        ...
                        local[base + t] = a as f32;
                        local[base + t + 1] = b as f32;
                        local[base + t + 2] = c as f32;
                        local[base + t + 3] = d as f32;
                        t += 4;
                    }
                    while t < tq {
                        local[base + t] = dot_i8(wrow, row(xi8, t)) as f32;
                        t += 1;
                    }
                }
                local
            ...
            .map(|&(o0, o1)| {
                ...
                for o in o0..o1 {
                    ...
                    while t + 6 <= tq {
                        ...
                        local[base + t + 2] = c as f32;
                        local[base + t + 3] = d as f32;
                        local[base + t + 4] = e as f32;
                        local[base + t + 5] = f as f32;
                        t += 6;
                    }
                    while t + 4 <= tq {
                        let (a, b, c, d) = dot_i8_4col(
            ...
            for workers in [1usize, avail.min(16)] {
                for (out, label) in [(5120usize, "mlp_0[1280,5120]"), (3840, "qkv[1280,3840]")] {
                    ...
                    for tq in [12usize, 64, 204] {
                        ...
                        println!(
                            "{label} tq={tq:3} {workers:2}t min-{reps}: 4col={b4:.4} ms  6col={b6:.4} ms  ({:.3}× {v})  byte-id={ident}",
                            b4 / b6
                        );
                    }
                }
...
#[cfg(not(all(target_arch = "x86_64", target_feature = "avx2")))]
fn main() {
    eprintln!("i8batch_6col_probe needs AVX2 (build with RUSTFLAGS=-Ctarget-cpu=native)");
}
