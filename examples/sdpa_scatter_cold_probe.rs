...
fn scatter(out: &mut [f32], o: &[f32], hh: usize, t: usize, d_head: usize, n_state: usize, chunks: usize) {
    let rows_per = if chunks == 0 { 1 } else { t.div_ceil(chunks.min(t).max(1)).max(1) };
...
fn main() {
    ...
    let pool_n = 30usize;
    println!("=== SDPA scatter chunk-count on COLD data (pool {}×{:.1}MB = {:.0}MB > 128MB L3) ===",
    ...
    let os: Vec<Vec<f32>> = (0..pool_n).map(|_| (0..hh * t * d_head).map(|_| nf()).collect()).collect();
    ...
    for (i, &ch) in chunk_opts.iter().enumerate() {
        let label = if ch == 0 { "row(1500)".to_string() } else { ch.to_string() };
        println!("  {label:>9} | {:>9.2} | {:>11.2}x | {:>8.1} | {}",
