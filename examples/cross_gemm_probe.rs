...
fn main() {
    ...
    let wk: Vec<Mat> = (0..n_layer).map(|l| fill(n_state, n_state, 0x100 + l as u64)).collect();
    let wv: Vec<Mat> = (0..n_layer).map(|l| fill(n_state, n_state, 0x200 + l as u64)).collect();
    ...
    for _ in 0..3 { black_box(seq()); black_box(band()); }

    let mut best_seq = f64::INFINITY;
    for _ in 0..iters { let t = Instant::now(); let r = seq(); best_seq = best_seq.min(t.elapsed().as_secs_f64()); black_box(r); }
    let mut best_band = f64::INFINITY;
    for _ in 0..iters { let t = Instant::now(); let r = band(); best_band = best_band.min(t.elapsed().as_secs_f64()); black_box(r); }

    println!("best-of-{iters}:");
    println!("  band-concurrency (current) : {:.3} ms", best_band * 1e3);
    println!("  sequential (full-parallel) : {:.3} ms   ({:.2}× vs band)", best_seq * 1e3, best_band / best_seq);
}
