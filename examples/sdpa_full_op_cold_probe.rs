...
fn gather(
    ...
    let n = if chunks == 0 {
        ...
        chunks.min(total).max(1)
    };
    let cr = total.div_ceil(n).max(1);
    dst.par_chunks_mut(cr * d_head)
        .enumerate()
        .for_each(|(c, blk)| {
            for (l, row) in blk.chunks_mut(d_head).enumerate() {
                ...
                let (h, i) = (r / t, r % t);
                row.copy_from_slice(
                    &src[i * n_state + h * d_head..i * n_state + h * d_head + d_head],
                );
            }
        });
}
...
fn main() {
    ...
    let qs: Vec<Vec<f32>> = (0..pool_n)
        ...
        .collect();
    let ks: Vec<Vec<f32>> = (0..pool_n)
        .map(|_| (0..one).map(|_| nf()).collect())
        .collect();
    let vs: Vec<Vec<f32>> = (0..pool_n)
        .map(|_| (0..one).map(|_| nf()).collect())
        .collect();
    ...
    println!(
        "  byte-exact (legacy vs 16): {bad} differing  [{}]",
        if bad == 0 { "IDENTICAL" } else { "DIVERGENT" }
    );
    println!("  legacy (per-head/per-row) : {:>8.1} µs/op", legacy * 1e6);
    println!(
        "  16 balanced chunks        : {:>8.1} µs/op  {:.3}x  [{}]",
        c16 * 1e6,
        legacy / c16,
        if c16 < legacy { "WIN" } else { "loss" }
    );
    println!(
        "  => per-LAYER SDPA speedup from the chunk flip (× {} encoder layers = e2e-relevant)",
        32
    );
}
