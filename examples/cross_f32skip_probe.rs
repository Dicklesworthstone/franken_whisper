...
fn build(cross_k: &[Vec<f32>], cross_v: &[Vec<f32>], n_layer: usize, n_head: usize,
         enc_frames: usize, n_state: usize, d_head: usize, need_f32: bool)
         -> Vec<(Vec<f32>, Vec<Float16>, Vec<f32>, Vec<Float16>)> {
    (0..n_layer * n_head).into_par_iter().map(|idx| {
        ...
        if need_f32 {
            ...
            for j in 0..enc_frames {
                let src = &ck[j * n_state + base..j * n_state + base + d_head];
                for &s in src { k_nat.push(Float16::from_f32(s)); }
            }
        }
        ...
        for j in 0..enc_frames {
            ...
            if need_f32 { vh[j * d_head..(j + 1) * d_head].copy_from_slice(src); }
            for (d, &s) in src.iter().enumerate() {
...
fn main() {
    ...
    println!("=== cross-build: all-4 buffers vs f16-only (skip unused f32 kh_t/vh), turbo 80 pairs ===");

    let mut s = 0x243F_6A88_85A3_08D3u64;
    let mut nf = || { s ^= s << 13; s ^= s >> 7; s ^= s << 17; ((s >> 40) as f32 / (1u64 << 24) as f32 - 0.5) };
    let cross_k: Vec<Vec<f32>> = (0..n_layer).map(|_| (0..enc_frames * n_state).map(|_| nf()).collect()).collect();
    let cross_v: Vec<Vec<f32>> = (0..n_layer).map(|_| (0..enc_frames * n_state).map(|_| nf()).collect()).collect();

    // byte-exactness of the f16 buffers (k_nat, v_t) across need_f32 true/false.
    let a = build(&cross_k, &cross_v, n_layer, n_head, enc_frames, n_state, d_head, true);
    let b = build(&cross_k, &cross_v, n_layer, n_head, enc_frames, n_state, d_head, false);
    let mut bad = 0usize;
    for (x, y) in a.iter().zip(&b) {
        if x.1.iter().zip(&y.1).any(|(p, q)| p.to_bits() != q.to_bits()) { bad += 1; }
        if x.3.iter().zip(&y.3).any(|(p, q)| p.to_bits() != q.to_bits()) { bad += 1; }
    }

    let run = |need_f32: bool| -> f64 {
        for _ in 0..3 { black_box(build(&cross_k, &cross_v, n_layer, n_head, enc_frames, n_state, d_head, need_f32)); }
        let mut best = f64::INFINITY;
        for _ in 0..iters {
            let t = Instant::now();
            let r = build(&cross_k, &cross_v, n_layer, n_head, enc_frames, n_state, d_head, need_f32);
    ...
    println!("  f16 buffers byte-exact across skip: {bad} differing pairs  [{}]", if bad == 0 { "IDENTICAL" } else { "DIVERGENT" });
    println!("  all-4 (current, need_f32=true)  : {:>8.2} ms/window", all4 * 1e3);
    println!("  f16-only (skip kh_t/vh)         : {:>8.2} ms/window  {:.2}x  [{}]",
        f16only * 1e3, all4 / f16only, if f16only < all4 { "WIN" } else { "loss" });
    println!("  => saved {:.2} ms/window when FW_CROSS_F16 on (default); kh_t is a large-stride transpose", (all4 - f16only) * 1e3);
}
