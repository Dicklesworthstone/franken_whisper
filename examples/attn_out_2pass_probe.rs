...
fn gemm_vpmaddwd(a: &[i8], w: &[i8], out: &mut [i32]) {
    ...
    out.par_chunks_mut(4 * N).enumerate().for_each(|(blk, cout)| {
        let r0 = blk * 4;
        if r0 + 4 > M {
            // tail: scalar
        ...
        unsafe {
            ...
            while o + 2 <= N {
                ...
                let mut k = 0;
                while k + 16 <= K {
                    let wv0 = _mm256_cvtepi8_epi16(_mm_loadu_si128(w0.add(k) as *const __m128i));
                    let wv1 = _mm256_cvtepi8_epi16(_mm_loadu_si128(w1.add(k) as *const __m128i));
                    for r in 0..4 {
                        let av = _mm256_cvtepi8_epi16(_mm_loadu_si128(ap[r].add(k) as *const __m128i));
                        acc[r * 2] = _mm256_add_epi32(acc[r * 2], _mm256_madd_epi16(av, wv0));
                        acc[r * 2 + 1] = _mm256_add_epi32(acc[r * 2 + 1], _mm256_madd_epi16(av, wv1));
                    }
                    k += 16;
                }
                for r in 0..4 {
            ...
            while o + 2 <= N {
                ...
                while k + 32 <= K {
                    ...
                    for r in 0..4 {
                        ...
                        acc[r * 2 + 1] = _mm256_add_epi32(acc[r * 2 + 1], _mm256_add_epi32(pa1, pb1));
                    }
                    k += 32;
                }
                for r in 0..4 {
...
fn main() {
    ...
    #[cfg(all(target_arch = "x86_64", target_feature = "avx2"))]
    {
        ...
        let mut ok = true;
        for &(r, o) in &[(0usize, 0usize), (1, 1), (7, 1023), (1499, 1279), (513, 777)] {
            let rf = ref_dot(&a, &w, r, o);
            if out_v[r * N + o] != rf || out_m[r * N + o] != rf {
                ok = false;
                println!("MISMATCH r={r} o={o}: ref={rf} vpmaddwd={} 2pass={}", out_v[r * N + o], out_m[r * N + o]);
            }
        }
        let equal = out_v == out_m;
        println!("i32-exact vpmaddwd==2pass==ref: {} (full-array equal: {})", ok, equal);
        ...
        println!(
            ...
            bm / bv,
            if bm < bv { "(2-pass FASTER)" } else { "(vpmaddwd faster ⇒ 2-pass DEAD)" }
        );
    }
}
