...
fn main() {
    ...
    let pool_n = 30usize;
    println!(
        "=== SDPA gather chunk flip: Pareto check across HOT vs COLD regimes (legacy=0 ⇒ per-head 20) ==="
    );
    ...
    println!(
        "  byte-exact across chunk counts: {bad} differing  [{}]",
    ...
    println!(
        "  => 16 vs legacy(20): HOT {:.2}x, COLD {:.2}x  ⇒ {}",
        hot[li] / hot[c16],
        cold[li] / cold[c16],
        if pareto {
            "PARETO-SAFE (16 wins in BOTH regimes)"
        } else {
            "NOT Pareto (regime-dependent)"
        }
    );
}
