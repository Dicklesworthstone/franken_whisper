...
fn main() {
    ...
    let wf16: Vec<Vec<f16>> = (0..n_layer * 2)
        .map(|i| fill_f32(n_state * n_state, 0x100 + i as u64).iter().map(|&v| f16::from_f32(v)).collect())
        .collect();
    ...
    println!("turbo cross proj: enc[{enc},{n_state}] × {} [{n_state},{n_state}] GEMMs", n_layer * 2);
    ...
    let maxd = yf16.iter().zip(&yf32.data).map(|(a, b)| (a - b).abs()).fold(0.0f32, f32::max);
    println!("max|Δ| f16-gemv vs f32-sgemm = {maxd:.3e}  (0 = bit-exact; >0 = NOT byte-exact to swap)");
    ...
    for _ in 0..3 { black_box(f16path()); black_box(f32path()); }
    let mut b16 = f64::INFINITY;
    for _ in 0..iters { let t = Instant::now(); let r = f16path(); b16 = b16.min(t.elapsed().as_secs_f64()); black_box(r); }
    let mut b32 = f64::INFINITY;
    for _ in 0..iters { let t = Instant::now(); let r = f32path(); b32 = b32.min(t.elapsed().as_secs_f64()); black_box(r); }

    println!("best-of-{iters} (8 GEMMs):");
    println!("  f16 gemv_f16_batch (real) : {:.3} ms", b16 * 1e3);
    println!("  f32 sgemm (dequant-once)  : {:.3} ms   ({:.2}× vs f16)", b32 * 1e3, b16 / b32);
}
