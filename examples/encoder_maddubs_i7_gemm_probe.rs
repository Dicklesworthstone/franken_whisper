...
mod x86_probe {
            ...
            while x + 32 <= k {
                ...
                for i in 0..8 {
                    acc[i] = _mm256_add_epi32(
                        ...
                        _mm256_madd_epi16(
                            _mm256_maddubs_epi16(
                                _mm256_loadu_si256(p[i].add(x) as *const __m256i),
                                wv,
                            ),
                            ones,
                        ),
                ...
                while o0 < n {
                    ...
                    while rr + 4 <= task_rows {
                        ...
                        while o + 2 <= pend {
                            ...
                            for i in 0..4 {
                                cblk[(rr + i) * n + o] =
                                    (raw[i] - off0) as f32 * sa[r0 + i] * sw[o];
                                cblk[(rr + i) * n + o + 1] =
    ...
    pub fn run() {
        ...
        println!(
            "(speedup = f32/variant; sat_diff MUST be 0 = maddubs-7bit is exact/non-saturating)"
        );
...
fn main() {
    if !std::is_x86_feature_detected!("avx2") {
        eprintln!("encoder_maddubs_i7_gemm_probe requires AVX2 support");
        return;
    }
    x86_probe::run();
}
...
#[cfg(not(target_arch = "x86_64"))]
fn main() {
    eprintln!("encoder_maddubs_i7_gemm_probe requires an x86_64 processor");
}
