...
fn scatter_pos_major(out: &mut [f32], o: &[f32], chunks: usize) {
    ...
    out.par_chunks_mut(rows_per * NS).enumerate().for_each(|(c, blk)| {
        let i0 = c * rows_per;
...
fn main() {
    ...
    let o: Vec<f32> = (0..HH * T * DH)
        .map(|_| { s = s.wrapping_mul(6364136223846793005).wrapping_add(1); (s >> 40) as f32 })
        .collect();
    ...
    for ch in [16usize, 0] {
        ...
        for _ in 0..reps {
            ...
            let t = std::time::Instant::now(); scatter_pos_major(&mut a, &o, ch); bp = bp.min(ms(t));
            for e in evict.iter_mut() { *e *= 1.0000001; }
            let t = std::time::Instant::now(); scatter_head_major(&mut b, &o, ch); bh = bh.min(ms(t));
        }
        ...
        println!(
            ...
            bh / bp,
            if bh < bp { "(head-major FASTER)" } else { "(pos-major faster ⇒ reorder DEAD)" }
        );
    }
