...
fn scores_f16_naive(k: &[u16], qh: &[f32], scale: f32, tk: usize, base: usize, out: &mut [f32]) {
    for (j, sj) in out.iter_mut().enumerate().take(tk) {
        let krow = &k[j * N_STATE + base..j * N_STATE + base + D_HEAD];
        let mut acc = 0.0f32;
        for (d, &qd) in qh.iter().enumerate() {
...
unsafe fn scores_f16_f16c(k: &[u16], qh: &[f32], scale: f32, tk: usize, base: usize, out: &mut [f32]) {
...
unsafe fn scores_f32_simd(k: &[f32], qh: &[f32], scale: f32, tk: usize, base: usize, out: &mut [f32]) {
    ...
    for (j, sj) in out.iter_mut().enumerate().take(tk) {
        ...
        let mut d = 0;
        while d < D_HEAD {
            a0 = _mm256_fmadd_ps(_mm256_loadu_ps(qs.as_ptr().add(d)), _mm256_loadu_ps(krow.add(d)), a0);
            a1 = _mm256_fmadd_ps(_mm256_loadu_ps(qs.as_ptr().add(d + 8)), _mm256_loadu_ps(krow.add(d + 8)), a1);
            a2 = _mm256_fmadd_ps(_mm256_loadu_ps(qs.as_ptr().add(d + 16)), _mm256_loadu_ps(krow.add(d + 16)), a2);
            a3 = _mm256_fmadd_ps(_mm256_loadu_ps(qs.as_ptr().add(d + 24)), _mm256_loadu_ps(krow.add(d + 24)), a3);
            d += 32;
        }
...
fn main() {
    ...
    let mut nf = || { st ^= st << 13; st ^= st >> 7; st ^= st << 17; ((st >> 40) as f32 / (1u64 << 24) as f32) * 2.0 - 1.0 };
    ...
    for &tk in &[64usize, 128, 256] {
        ...
        let kh: Vec<u16> = kf.iter().map(|&x| unsafe {
        ...
        unsafe { scores_f16_f16c(&kh, &qh, scale, tk, 0, &mut o2); }
        let diff = o1.iter().zip(&o2).filter(|(a, b)| a.to_bits() != b.to_bits()).count();

        let bench = |f: &mut dyn FnMut()| { for _ in 0..500 { f(); } let mut best = f64::INFINITY; for _ in 0..7 { let t = Instant::now(); for _ in 0..iters { f(); } best = best.min(t.elapsed().as_secs_f64()); } best / iters as f64 * 1e6 };
        // sum over all heads (realistic per-token self-attn cost)
        let tf = bench(&mut || { for h in 0..N_HEAD { scores_f32(&kf, &qh, scale, tk, h * D_HEAD, &mut o0); black_box(&o0[0]); } });
        let tn = bench(&mut || { for h in 0..N_HEAD { scores_f16_naive(&kh, &qh, scale, tk, h * D_HEAD, &mut o1); black_box(&o1[0]); } });
        let tc = bench(&mut || { for h in 0..N_HEAD { unsafe { scores_f16_f16c(&kh, &qh, scale, tk, h * D_HEAD, &mut o2); } black_box(&o2[0]); } });
        let mut o3 = vec![0f32; tk];
        unsafe { scores_f32_simd(&kf, &qh, scale, tk, 0, &mut o3); }
        scores_f32(&kf, &qh, scale, tk, 0, &mut o0);
        let mut maxrel = 0f32;
        for (a, b) in o0.iter().zip(&o3) { let d = (a - b).abs() / a.abs().max(1e-6); maxrel = maxrel.max(d); }
        let ts = bench(&mut || { for h in 0..N_HEAD { unsafe { scores_f32_simd(&kf, &qh, scale, tk, h * D_HEAD, &mut o3); } black_box(&o3[0]); } });

        println!("=== self-attn scores, ALL {N_HEAD} heads, tk={tk} (cache {} KB f32 / {} KB f16) ===",
            tk * N_STATE * 4 / 1024, tk * N_STATE * 2 / 1024);
        println!("  f16-naive vs f16-F16C byte-diff: {diff}/{tk}  [{}]", if diff == 0 { "IDENTICAL" } else { "DIVERGENT" });
        println!("  f32-KV scalar dot        : {tf:>7.2} µs/token   1.00×");
        println!("  f16-KV naive to_f32/elem : {tn:>7.2} µs/token   {:.2}×  (memory's dead path)", tf / tn);
        println!("  f16-KV F16C batch dequant: {tc:>7.2} µs/token   {:.2}×  [{}]", tf / tc,
            if tc < tf * 0.9 { "WIN vs f32 — the missing ingredient works" } else { "no win vs f32" });
        println!("  f32-KV SIMD 4-accum dot  : {ts:>7.2} µs/token   {:.2}×  [{}] max-rel-Δ={maxrel:.1e} (non-byte-exact)", tf / ts,
            if ts < tf * 0.7 { "WIN — latency-bound chain broken; needs transcript A/B" } else { "marginal" });
    }
}
