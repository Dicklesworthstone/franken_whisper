...
fn main() {
    ...
    let mut sum_indep = 0.0f64; let mut sum_ef = 0.0f64;
    let mut max_indep = 0.0f64; let mut max_ef = 0.0f64;
    let mut ef_closer = 0u64; let mut indep_closer = 0u64; let mut n = 0u64;
    ...
    for _c in 0..ncol {
        ...
        for v in col.iter_mut() { *v = rng.gauss(0.06); }
        let oi = (rng.u32() as usize) % inp; col[oi] *= 6.0;
        ...
        for _a in 0..nact {
            ...
            for v in a.iter_mut() { *v = rng.gauss(1.0); }
            let aamax = a.iter().map(|v| v.abs()).fold(0.0f32, f32::max).max(1e-9);
            let sa = aamax / 127.0; let ainv = 1.0 / sa;
            let qa: Vec<i32> = a.iter().map(|&v| (v * ainv).round().clamp(-127.0, 127.0) as i32).collect();

            // f32 truth (the golden dot, full precision f32 operands)
            let mut truth = 0.0f64;
            for i in 0..inp { truth += (col[i] as f64) * (a[i] as f64); }

            // int8 indep: sw*sa * Σ qw_i*qa
            let mut d_i = 0i64; let mut d_e = 0i64;
            for i in 0..inp {
            ...
            sum_indep += ei; sum_ef += ee;
            if ei > max_indep { max_indep = ei; }
            if ee > max_ef { max_ef = ee; }
            if ee < ei { ef_closer += 1; } else if ei < ee { indep_closer += 1; }
            n += 1;
        }
    }
    println!("samples                 = {}", n);
    println!("indep  mean|err|        = {:.4e}   max = {:.4e}", sum_indep / n as f64, max_indep);
    println!("EF     mean|err|        = {:.4e}   max = {:.4e}", sum_ef / n as f64, max_ef);
    let r = (sum_indep / n as f64) / (sum_ef / n as f64);
    println!("indep/EF mean-err ratio = {:.3}x   (>1 => EF MORE accurate => promising)", r);
    println!("EF closer to truth      = {} / {}  ({:.1}%)", ef_closer, n, 100.0*ef_closer as f64/n as f64);
    println!("indep closer            = {} / {}  ({:.1}%)", indep_closer, n, 100.0*indep_closer as f64/n as f64);
}
