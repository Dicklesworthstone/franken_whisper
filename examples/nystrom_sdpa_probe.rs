...
fn tensor_vec(model: &GgmlModel, name: &str) -> Vec<f32> {
    model.tensor_f32(name).expect("vec").1
}
...
fn main() {
    let wav = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "tests/fixtures/native/jfk.wav".into());
    ...
    let l: usize = std::env::var("FW_ENCODER_LAYERS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(32);
    println!(
        "=== Nyström SDPA probe | {wav}x{repeat} ({audio_sec:.1}s) depth={l} n_head={n_head} ==="
    );
    println!(
        "relerr = ‖O_nys−O_exact‖/‖O_exact‖ (encoder tolerates ≪1%/layer); speed = exact_per_head ÷ nystrom"
    );
    {
        let layer = l - 1;
        ...
        for &h in &[0usize, n_head / 2, n_head - 1] {
            ...
            for i in 0..n {
                qh[i * d..(i + 1) * d]
                    .copy_from_slice(&q.data[i * n_state + b..i * n_state + b + d]);
                khd[i * d..(i + 1) * d]
                    .copy_from_slice(&kk.data[i * n_state + b..i * n_state + b + d]);
                vhd[i * d..(i + 1) * d]
                    .copy_from_slice(&vv.data[i * n_state + b..i * n_state + b + d]);
            }
            ...
            for &m in &[64usize, 128] {
                ...
                let mut f = qk_scaled(&qh, &kt, n, m, d, scale);
                softmax_rows(&mut f, n, m); // [n,m]
                let mut a = qk_scaled(&qt, &kt, m, m, d, scale);
                softmax_rows(&mut a, m, m); // [m,m]
                let mut bmat = qk_scaled(&qt, &khd, m, n, d, scale);
                softmax_rows(&mut bmat, m, n); // [m,n]
                let ap = pinv(&a, m); // [m,m]
                ...
                for i in 0..n * d {
                    diff[i] = o_nys[i] - o_exact[i];
                }
                let relerr = frob(&diff) / frob(&o_exact).max(1e-12) * 100.0;
                line += &format!(
                    " m{m}: {t_nys:>7.0}us {:>5.2}x relerr={relerr:>6.2}% |",
                    t_exact / t_nys
                );
            }
            println!("{line}");
    ...
    println!(
        "VIABLE iff (relerr ≪1%/layer, esp. at L32) AND (speed >1× vs exact per-head). Else owner-gated/reject."
    );
}
