...
mod x86_probe {
    ...
    unsafe fn max_avx2(logits: &[f32]) -> f32 {
        // SAFETY: callers gate AVX2. Every vector load is guarded by the remaining
        // slice length, and the scalar tail uses bounds-checked indexing.
        unsafe {
            let n = logits.len();
            let p = logits.as_ptr();
...
fn main() {
    if !std::is_x86_feature_detected!("avx2") {
        eprintln!("logit_max_reduce_probe requires AVX2 support");
        return;
    }
    x86_probe::run();
}
...
fn main() {
    eprintln!("logit_max_reduce_probe requires an x86_64 processor");
}
