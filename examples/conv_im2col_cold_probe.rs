...
fn im2col(
    ...
    out.par_chunks_mut(rows_per * patch)
        .enumerate()
        .for_each(|(c, band)| {
            let o0 = c * rows_per;
...
fn main() {
    ...
    println!(
        "=== conv2 im2col chunk-count on COLD data (t_out={t_out}, patch={patch}, pool {} = {:.0}MB) ===",
        pool_n,
        pool_n as f64 * (xlen + outlen) as f64 * 4.0 / 1e6
    );
    ...
    let xs: Vec<Vec<f32>> = (0..pool_n)
        .map(|_| (0..xlen).map(|_| nf()).collect())
        .collect();
    ...
    println!(
        "  (the conv encode path uses ~worker_count()=~32 bands; >1.00x means a lower count is faster)"
    );
}
