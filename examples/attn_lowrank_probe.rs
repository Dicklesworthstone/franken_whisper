...
fn main() {
    ...
    println!(
        "=== encoder attn score-matrix rank | depth={l} attn-layer={layer} | real_frames={real} n_head={n_head} d_head={d_head} ==="
    );
    ...
    for &h in &heads {
        ...
        for &r in &[8usize, 16, 32, 64, 128] {
            if r < real {
                line += &format!("r{r}={:.1}% ", captured(&scores, real, r) * 100.0);
            }
        }
        println!("{line}");
    }
    println!(
        "  (rank r captures ≥99% at r≪{real} ⇒ Nyström/linear attention viable; near-full-rank ⇒ dead)"
    );
}
