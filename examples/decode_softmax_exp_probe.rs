...
fn bench(
    ...
    cols: usize,
    iters: usize,
    kernel: &dyn Fn(&mut [f32]),
    base: &[f32],
) -> f64 {
    let mut data = base.to_vec();
    for _ in 0..3 {
        for r in data.chunks_mut(cols) {
            kernel(r);
        }
...
fn main() {
    ...
    println!(
        "=== decode softmax exp: franken scalar (libm) vs AVX2 poly, per-token turbo shapes ==="
    );
    let mut tot_scalar = 0.0;
    let mut tot_simd = 0.0;
    let mut maxd = 0.0f32;
    for (name, (rows, cols)) in [
        ("CROSS (4×20 heads × 1500)", cross),
        ...
        println!("  {name}:");
        let ts = bench(
            "scalar softmax_rows (libm)",
            rows,
            cols,
            iters,
            &|r| softmax_row_scalar(r),
            &base,
        );
        #[cfg(target_arch = "x86_64")]
        let tv = bench(
            "AVX2 poly-exp softmax",
            rows,
            cols,
            iters,
            &|r| softmax_row_simd(r),
            &base,
        );
    ...
    println!("  ------------------------------------------------------------");
    println!(
        "  PER-TOKEN TOTAL scalar (libm) : {:>8.1} µs/token",
        tot_scalar * 1e6
    );
    println!(
        "  PER-TOKEN TOTAL AVX2 poly      : {:>8.1} µs/token  ({:.2}x)",
        tot_simd * 1e6,
        tot_scalar / tot_simd
    );
    println!(
        "  saved                          : {:>8.1} µs/token",
        (tot_scalar - tot_simd) * 1e6
    );
    println!("  faithfulness: max|Δ|={maxd:.3e} (poly ≠ libm ⇒ NON-byte-exact, owner-gated)");
    println!(
        "  NOTE: cross-attn softmax dominates; in TIMESTAMP mode decode is NOT pipelining-hidden."
    );
}
