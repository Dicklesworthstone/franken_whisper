...
fn captured_energy(m: &Mat, rows: usize, r: usize) -> f64 {
    ...
    for j in 0..r {
        ...
        for q in &qcols {
            ...
            for (vi, &qi) in v.iter_mut().zip(q) { *vi -= d * qi; }
        }
        let nm = norm(&v);
        if nm > 1e-5 { let inv = (1.0 / nm) as f32; for vi in v.iter_mut() { *vi *= inv; } qcols.push(v); }
    }
    let rk = qcols.len();
    let mut qt = vec![0.0f32; rk * rows];
    for (j, qc) in qcols.iter().enumerate() { for i in 0..rows { qt[j * rows + i] = qc[i]; } }
    let proj = nn::matmul(&Mat::from_vec(rk, rows, qt), &sub).unwrap();
    let pf: f64 = proj.data.iter().map(|&x| (x as f64) * (x as f64)).sum::<f64>().sqrt();
    let sf: f64 = sub.data.iter().map(|&x| (x as f64) * (x as f64)).sum::<f64>().sqrt();
    pf / sf
}

fn main() {
    let wav = std::env::args().nth(1).unwrap_or_else(|| "tests/fixtures/native/jfk.wav".to_string());
    ...
    let model = GgmlModel::load(&path).and_then(LoadedModel::from_ggml).expect("load turbo");
    ...
    println!("=== encoder frame redundancy | wav={wav} x{repeat} ({audio_sec:.1}s) depth={depth} | n_ctx={n_ctx} real_frames={real} n_state={} ===", out.cols);
    ...
    for i in 0..real.saturating_sub(1) {
        ...
        if na > 0.0 && nb > 0.0 { sims.push(dot(a, b) / (na * nb)); }
    }
    let mut sorted = sims.clone(); sorted.sort_by(|x, y| x.partial_cmp(y).unwrap());
    let mean = sims.iter().sum::<f64>() / sims.len() as f64;
    let median = sorted[sorted.len() / 2];
    let frac = |t: f64| sims.iter().filter(|&&s| s > t).count() as f64 / sims.len() as f64 * 100.0;
    println!("  (a) adjacent cos-sim: mean={mean:.3} median={median:.3} | >0.8:{:.0}% >0.9:{:.0}% >0.95:{:.0}% >0.99:{:.0}%",
             frac(0.8), frac(0.9), frac(0.95), frac(0.99));

    // (b) effective rank of the real-frame block (captured energy at r).
    for &r in &[32usize, 64, 128, 256] {
        if r < real { println!("  (b) rank-{r:<4} captured energy = {:.2}%  (of {real} frames' span)", captured_energy(&out, real, r) * 100.0); }
    }

    // (c) ToMe 2× proxy: average adjacent pairs, reconstruction relerr over real frames.
    let mut err2 = 0.0f64; let mut sig2 = 0.0f64;
    let mut i = 0;
    while i + 1 < real {
        let a = &out.data[i * cols..(i + 1) * cols];
        let b = &out.data[(i + 1) * cols..(i + 2) * cols];
        for k in 0..cols {
            let avg = 0.5 * (a[k] + b[k]);
            let ea = a[k] as f64 - avg as f64; let eb = b[k] as f64 - avg as f64;
            err2 += ea * ea + eb * eb;
            sig2 += (a[k] as f64) * (a[k] as f64) + (b[k] as f64) * (b[k] as f64);
        }
        i += 2;
    }
    println!("  (c) uniform 2× adjacent-merge reconstruction relerr = {:.2}%  (transcript tolerates <<1%)", (err2 / sig2).sqrt() * 100.0);
}
