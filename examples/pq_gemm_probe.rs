...
fn pq_time(
    x: &[f32], m: usize, in_d: usize, out_d: usize,
    d: usize, k: usize, centroids: &[f32], codes: &[u8], iters: usize,
) -> f64 {
    let n_sub = in_d / d;
    let run = || {
...
fn frob(v: &[f32]) -> f64 { v.iter().map(|&x| (x as f64) * (x as f64)).sum::<f64>().sqrt() }
...
fn vq_relerr(wt: &[f32], in_d: usize, out_d: usize, d: usize, k: usize, iters: usize) -> f64 {
    ...
    for s in 0..n_sub {
        ...
        for c in 0..k {
            cen[c * d..c * d + d].copy_from_slice(&pts[(c * (out_d / k)) * d..(c * (out_d / k)) * d + d]);
        }
        ...
        for _ in 0..iters {
            ...
            for o in 0..out_d {
                ...
                for c in 0..k {
                    ...
                    let mut dd = 0.0f32;
                    for j in 0..d { let e = p[j] - cc[j]; dd += e * e; }
                    if dd < bd { bd = dd; bi = c; }
                }
                assign[o] = bi;
            }
            ...
            for o in 0..out_d {
                let c = assign[o];
                cnt[c] += 1;
                for j in 0..d { cen[c * d + j] += pts[o * d + j]; }
            }
            for c in 0..k {
                if cnt[c] > 0 { let inv = 1.0 / cnt[c] as f32; for j in 0..d { cen[c * d + j] *= inv; } }
            }
        }
        // reconstruction error for this subspace
        for o in 0..out_d {
            let c = assign[o];
            for j in 0..d { let e = pts[o * d + j] - cen[c * d + j]; err2 += (e as f64) * (e as f64); }
        }
    }
    err2.sqrt() / wt_frob
}
...
fn bench(model: &GgmlModel, label: &str, name: &str, seq: usize, configs: &[(usize, usize)], iters: usize) {
    ...
    let mut af = || { s = s.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407); ((s >> 40) as f32 / (1u64 << 24) as f32) - 0.5 };
    let x: Vec<f32> = (0..seq * in_d).map(|_| af()).collect();
    let x_mat = Mat::from_vec(seq, in_d, x.clone());

    let mut bbase = f64::INFINITY;
    for _ in 0..3 { black_box(nn::matmul(&x_mat, &wt_mat).unwrap()); }
    for _ in 0..iters { let t = Instant::now(); black_box(nn::matmul(&x_mat, &wt_mat).unwrap()); bbase = bbase.min(t.elapsed().as_secs_f64()); }
    println!("\n{label}  W_t[{in_d},{out_d}] seq={seq} ({gflop:.2} GFLOP)  FMA baseline {:.3} ms {:.0} GF/s @ {}t",
             bbase * 1e3, gflop / bbase, rayon::current_num_threads());
    for &(d, k) in configs {
        if in_d % d != 0 { println!("  D={d} K={k}: skip (in%D!=0)"); continue; }
        let n_sub = in_d / d;
        // random centroids + codes for the SPEED leg (speed is data-independent).
        let centroids: Vec<f32> = (0..n_sub * k * d).map(|_| af()).collect();
        let codes: Vec<u8> = (0..out_d * n_sub).map(|_| (((af() + 0.5) * k as f32) as usize % k) as u8).collect();
        ...
        println!("  D={d:<2} K={k:<4}: {:.3} ms  {:.2}x speed (per-row table {:.0} KB) | VQ relerr={:.2}%  ({:.2} bits/weight)",
                 tpq * 1e3, bbase / tpq, tbl_kb, relerr * 100.0, bits_per_w);
    }
}
...
fn main() {
    ...
    println!("=== PQ (table-lookup) GEMM vs FMA, encoder layer {layer}, real turbo weights @ {}t ===", rayon::current_num_threads());
    println!("speed = FMA / PQ (higher=better); VQ relerr = real-weight k-means reconstruction; L1 data cache = 32 KB");
    let cfgs = [(4usize, 16usize), (8, 16), (8, 256), (4, 256)];
    let p = |s: &str| format!("encoder.blocks.{layer}.{s}");
    bench(&model, "attn.query", &p("attn.query.weight"), 1500, &cfgs, iters);
