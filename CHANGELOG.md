...
- **Live-streaming documentation wave (bd-rt-docs-7aa6).** README gained a "Real-Time Streaming (`fw robot listen`)" section (quickstart, agent integration loops, event contract with lifecycle invariants, emission-policy guide, confirm lane, persistence, and latency expectations citing the 2026-08-23 PERF_LEDGER campaign — including its failed A/A nulls and therefore no comparative claims); the Robot Event Catalog, JSON Schema Reference, `robot schema` key dump, and Use Cases Gallery now cover the listen family honestly (gallery rows no longer oversell batch stage-event streaming as live); Pre-Flight checklist gained live-session sizing (dual-model RAM, TCC pre-authorization, Rayon pool). New `docs/realtime-streaming.md` documents the architecture, design rationale (AlignAtt-over-LocalAgreement evidence, dynamic audio_ctx tradeoff, bounded-buffer-vs-seek-resume, append-only contract vs. Deepgram/AssemblyAI/OpenAI interim-revision semantics), tunables, failure-mode taxonomy, and scope boundaries; adaptive controllers are documented as planned-only (open bead bd-rt-adaptive-contract-yw68), not shipped. `docs/operational-playbook.md` gained a Live Session Operations section.
