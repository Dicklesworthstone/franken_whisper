...
**Land-or-dig result (surfaced with measured-rate arithmetic; live probe blocked by shared-box load).** AGENT_NAME=BlackThrush. `project_sampler_exp_measured` sized ONLY the sampler's vocab-wide exp (51866/token, libm expf ≈150 µs = ~2.9 ns/exp; AVX2 poly ≈9 µs = ~0.17 ns/exp, 16.7×) and concluded "non-byte-exact + pipelining-HIDDEN ⇒ ~0 realistic e2e". Auditing the decoder softmax sites shows that conclusion **undercounts the exp workload and misses a mode**:

- **Cross-attention** (`cross_attention`, decoder.rs:1281 & 1312) softmaxes over all 1500 encoder frames per head via scalar `nn::softmax_rows` (libm `.exp()`): 4 layers × 20 heads × 1500 = **120000 exp/token** — **2.3× the sampler**, the single largest exp consumer in decode. At the measured libm rate that is ~348 µs/token; an AVX2 poly would cut it to ~20 µs.
- **Self-attention** adds 4 × 20 × seq_len (grows) more; **sampler** adds 51866. Per-token decode exp ≈ **120K (cross) + ~10K (self) + 51.8K (sampler) ≈ 182K exp/token** ⇒ ~**530 µs/token** libm vs ~30 µs poly (measured rates).
- **Mode gap:** the "pipelining-hidden ⇒ ~0 e2e" reasoning only holds in **no_timestamps** mode (decode ∥ encode, [[project_window_pipelining_lever]]). In **timestamp mode** (data-dependent seek — pipelining DEAD, measured) decode is fully exposed, so this ~500 µs/token is real e2e: at ~1.8 ms/token turbo decode that is ~28% of decode ≈ **~5% e2e in timestamp mode**.

**Still owner-gated, still doesn't LAND byte-exact** (poly ≠ libm; franken's libm already diverges from ggml_v_expf, so the faithfulness target is franken's own golden — needs the turbo model to confirm the poly is transcript-neutral, which this box lacks). The max/sum reductions and normalize are byte-locked (float reorder) and cheap next to the exp, so there is NO byte-exact sub-lever — the whole win is the non-byte-exact exp. But this **corrects the EV**: the SIMD-exp lever is ~5% e2e in timestamp mode (not "~0"), dominated by cross-attn softmax, and should be the FIRST owner-gated lever revisited when a quiet box + model are available. Harness: `examples/decode_softmax_exp_probe` (scalar `softmax_rows` vs AVX2 Cephes-poly softmax on the real per-token shapes; blocked from executing this cycle only by remote-build contention). Ratio vs OpenAI-Whisper unchanged (~1.2× ts / ~1.68–1.8× no_ts; default untouched, byte-identical).
