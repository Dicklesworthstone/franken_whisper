...
**Land-or-dig result.** AGENT_NAME=BlackThrush. Auditing the decode self-attention (`attention_decode_step`, the DEFAULT f32 path — f16 KV was measured 2× slower, [[project_self_attn_kv_cache_lever]]): the scores `Σ_d qh[d]·(k[j][d]·scale)` is a SCALAR contiguous dot (float reduction ⇒ no autovec; the 2-row register-blocked GEMV only ever landed on the f16 path). A **byte-exact** vectorization exists — swap to d-outer/j-inner so each `scores[j] += qh[d]·(k[j][d]·scale)` still accumulates d-ascending (bit-identical sum order) while the inner j-loop autovectorizes over the independent output slots (harness `examples/self_attn_scores_probe`, byte-diff asserted 0; timing this cycle blocked by repeated shared-box RCH-E309 artifact-retrieval failures).

**But the EV ceiling makes it not worth blocking on:** self-attn is ~10.7% of decode ([[project_self_attn_kv_cache_lever]]), of which the scores dot is a fraction (the rest is KV read + softmax + the score·V AXPY, already `axpy_f32_into`), and the self KV cache is SHORT (≤ n_text_ctx=224 tokens ⇒ ~2.3 MB K+V, L3-resident) so the dot is cache-fed, not DRAM-bound — even fully eliminating the scores dot is **< 0.3% e2e** (and decode is pipelining-hidden except in timestamp mode). The loop-swap also trades the scalar's contiguous d_head read for a strided (n_state-stride) k read, so the cache penalty likely eats the vectorization at these lengths.
