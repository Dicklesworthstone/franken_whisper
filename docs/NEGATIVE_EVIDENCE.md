...
---
## 2026-07-05 - BlackThrush: CORRECTION (methodology) to the logits-tax entry below — **the "63% of per-token time" figure is a WARM MIXED-REGIME artifact, not the honest per-token fraction. MEASURED topology (`lscpu`): this box has 4×32 MiB L3 (one per CCD, cpus 0-7,32-39 per instance). The first version of `self_draft_logits_tax_probe` reused ONE weight matrix per op in a warm min-of-7 loop, so the 5–6 MB layer weights became L3-resident (measured 73–78 GB/s) while the 66 MB logits weight (16.6 MB/CCD, near the 32 MB CCD-L3 limit) stayed partially DRAM-cold (35 GB/s) — different cache regimes, which INFLATES the logits fraction. Regime-controlled (all ops DRAM-bound) the ratio ≈ the byte ratio 66/158 = **~42%**, not 63%. Rewrote the probe to measure BOTH warm and a DRAM-cold rotating-pool regime (>L3), applying the project's own "bench weight-streaming kernels COLD" lesson ([[project_draft_decoding_amortization]]) that the first version violated. A clean cold run was interrupted before capture, so the exact cold figure is pending — but the byte-ratio floor (42%) and the direction are settled by topology. Probe-only change, byte-identical default; ratio vs OpenAI-Whisper unchanged (~1.2× ts / ~1.68–1.8× no_ts).**

**Impact on the verdict (holds, but softened):** at the honest cold ~42% logits fraction, a 1-layer self-draft costs ~0.57× a full token (not 0.72×) → K=4 break-even ~56% accept (not 72%). So layer-skip self-speculation is structurally WEAK in both regimes but LESS dead than the warm number implied. The regime-invariant conclusion stands: **a real drafter must cut the LOGITS cost (smaller-vocab / reduced-head), which layer-skip does not.** The self_draft_logits_tax_probe now reports warm+cold side by side so the regime is explicit; don't quote the warm 63% as the per-token fraction.
