...
Portions of `src/sortformer_inference.rs` are a modified, safe Rust
translation of the selection logic in LLVM libc++ 15.0.7
`include/__algorithm/nth_element.h` and `include/__algorithm/sort.h`. They are
used solely to reproduce the pinned PyTorch 2.7.1 CPU `topk(sorted=false)` tie
selection required by the authenticated Streaming Sortformer parity contract.

The upstream files are licensed under Apache License 2.0 with LLVM Exceptions.
The translated portions have been changed for Rust indexing, checked bounds,
typed errors, and the model's fixed descending score comparator.
