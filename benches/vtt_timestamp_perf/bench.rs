// Parallel to benches/srt_timestamp_perf: measure the same allocation-free
// Display-timestamp lever (landed for SRT in 6de0c5e) applied to the VTT writer,
// whose `format_timestamp_vtt` still returns a `format!`-allocated `String`.
// VTT differs from SRT only in the `.` millisecond separator, the `WEBVTT`
// header, and the absence of a per-cue index line.
...
fn main() {
    ...
    assert_eq!(actual, expected, "VTT bytes changed");
    ...
    println!(
        "vtt_timestamp segments={} bytes={} samples={} iterations={} null_p10={:.6} null_median={:.6} null_p90={:.6} allocating_median_ns={} inline_median_ns={} speedup_p10={:.6} speedup_median={:.6} speedup_p90={:.6} wins={}/{}",
