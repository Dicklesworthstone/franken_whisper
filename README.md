...
- **Paired null (A/A) control, same binary** — every new A/B result must carry an identity null in the same invocation, with order-interleaved arms, so host contention and run-order bias cannot masquerade as a speedup. The resurrection audit identifies legacy rows that predate this contract instead of pretending they complied.
